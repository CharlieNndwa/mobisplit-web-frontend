...
<div className="relative z-10 w-full max-w-[1200px] mx-auto px-4 sm:px-6 xl:px-12 my-auto flex flex-col items-start text-left pt-6 pb-12">
  
  {/* Pitch Deck Core Tagline Block */}
  <div className="inline-block border border-zinc-800 bg-zinc-950/80 backdrop-blur-md px-3 py-1.5 mb-4 sm:mb-6 hero_text">
    <p className="text-[10px] sm:text-xs font-mono uppercase tracking-[0.2em] text-yellow-500">
      Same Direction • Shared Fare • Smart Savings
    </p>
  </div>

  {/* High-Impact Heading Matrix */}
  <h1 className="font-black text-3xl sm:text-6xl lg:text-7xl uppercase tracking-tight leading-[1.0] sm:leading-[0.95] text-white hero_text mb-4 sm:mb-6安全 max-w-[900px]">
  ...
  <p className="text-sm sm:text-xl text-zinc-300 font-normal max-w-[620px] leading-relaxed hero_text_description mb-8 sm:mb-10">
  ...
  {/* 🧊 Fixed Positioning & Mobile Isolation Container */}
  <div className="relative hero_buttons z-30 w-full sm:w-auto block">
    <button 
      onClick={() => setShowApps(!showApps)}
      onMouseEnter={() => setShowApps(true)}
      className="group flex items-center justify-between gap-4 sm:gap-6 border border-blue-500/40 bg-gradient-to-r from-blue-950 via-[#0a122c] to-zinc-950 text-white font-bold uppercase tracking-wider text-xs sm:text-sm px-5 sm:px-6 py-3.5 sm:py-4 transition-all duration-300 hover:border-blue-400 hover:shadow-[0_0_20px_rgba(59,130,246,0.3)] active:scale-[0.98] w-full sm:w-auto rounded-none"
    >
    ...
    {/* Dropdown Panel - Fixed responsive horizontal clipping layout rules */}
    <div 
      onMouseLeave={() => setShowApps(false)}
      className={`absolute left-0 right-0 sm:right-auto mt-2 w-full sm:w-80 bg-[#040814]/95 border border-blue-900/50 p-2.5 transition-all duration-300 flex flex-col gap-2 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] ${
    ...
    >
      ...
      <a 
        ...
        target="_blank" 
        rel="noreferrer"
        className="group/item flex items-center gap-4 p-2.5 border border-blue-950/40 bg-[#070d20]/60 hover:bg-[#0c1635]/80 hover:border-blue-700/50 transition-all duration-200 rounded-none relative overflow-hidden"
      >
        ...
        <div className="text-left relative z-10">
          <p className="text-[8px] text-blue-400/70 uppercase tracking-widest font-semibold leading-none mb-1">Download on the</p>
          <p className="text-sm font-bold text-white font-sans tracking-wide leading-tight">App Store</p>
        </div>
      </a>
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-r from-blue-600/0 via-blue-500/[0.03] to-blue-500/0 opacity-0 group-hover/item:opacity-100 transition-opacity" />
        <div className="relative w-9 h-9 flex-shrink-0 bg-[#0c122c] rounded-md overflow-hidden p-1">
          <img 
        ...
        <div className="text-left relative z-10">
          <p className="text-[8px] text-blue-400/70 uppercase tracking-widest font-semibold leading-none mb-1">Get it on</p>
          <p className="text-sm font-bold text-white font-sans tracking-wide leading-tight">Google Play</p>
        </div>
      </a>
    </div>
  </div>
